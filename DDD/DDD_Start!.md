...
이 상황에서 실제 결제 금액을 계산해야 하는 애그리거트는 어디일까?  
생각해볼 수 있는 방법 중 하나는 주문 애그리거트가 필요한 데이터를 모두 가지도록 한 뒤 할인 금액 계산 책임을 주문 애그리거트에서 담당하는 것이다.  

하지만 과연 결제 금액 계산 로직이 주문 애그리거트의 책임이 맞을까?  
예를 들어 특별 감사 세일로 전 품목에 대해 2%의 추가 할인이 들어가게 된다면, 이런 할인 정책은 주문 애그리거트와 크게 관련이 없음에도 불구하고 결제 금액 계산 로직이 있다는 이유만으로 주문 애그리거트의 코드를 수정해야 한다.  

이렇게 한 애그리거트에 넣기 애매한 도메인 로직을 억지로 특정 애그리거트에 넣어서는 안 된다.  
...
이를 해결하기 위한 좋은 방법은 바로 도메인 서비스를 별도로 구현하는 것이다.  
...
특히 도메인 로직을 외부 시스템이나 별도 엔진(ex. 룰 엔진)을 이용해서 구현해야 할 경우에 인터페이스와 클래스를 분리하게 된다.  
...
각각 개념적으로는 동일한 애그리거트이지만 물리적으로는 다른 애그리거트를 사용한다.  
이 상황에서 두 스레드는 각각 트랜잭션을 커밋할 때 수정한 내용을 DBMS에 반영한다.  
즉, 배송 상태도 바뀌고 배송지 정보도 바뀌게 되어 애그리거트의 일관성이 깨지게 된다.  
...
애그리거트에 대해 사용할 수 있는 대표적인 트랜잭션 처리 방식에는 `선점 잠금(Pessimistic)`과 `비선점 잠금(Optimistic)`의 두 가지 방식이 있다.  

> Pessimistic Lock과 Optimistic Lock을 비관적 잠금과 낙관적 잠금이라는 용어로 표현하기도 하지만 의미가 좀 더 가까운 선점 잠금과 비선점 잠금이란 용어를 사용한다.

### 선점 잠금

선점 잠금(Pessimistic Lock)은 먼저 애그리거트를 구한 스레드가 애그리거트 사용이 끝날 때까지 다른 스레드가 해당 애그리거트를 수정하는 것을 막는 방식이다.  

스레드1이 먼저 선점 잠금 방식으로 애그리거트를 구하면 애그리거트에 대한 잠금을 걸고, 스레드2는 해당 잠금이 해제될 때까지 블로킹된다.  
...
다음과 같은 순서로 두 스레드가 잠금 시도를 한다고 가정해 보자.  

- 스레드1 : A 애그리거트 선점 잠금 구함
- 스레드2 : B 애그리거트 선점 잠금 구함
- 스레드1 : B 애그리거트 선점 잠금 시도
- 스레드2 : A 애그리거트 선점 잠금 시도

이 순서에 따르면 두 스레드는 서로 상대방 스레드가 먼저 선점한 잠금을 구할 수 없어 데드락 상태에 빠지게 된다.  
...
JPA의 javax.persistence.lock.timeout 힌트는 잠금 대기 시간을 밀리초 단위로 지정하고, 지정한 시간 내에 잠금을 구하지 못하면 예외를 발생시킨다.  
힌트를 사용할 때 주의할 점은 DBMS에 따라 힌트가 적용되지 않을 수 있다는 것이다.  
따라서 사용 중인 DBMS가 관련 기능을 지원하는지 확인해야 한다.  
